/**
 * Facebook FCA Bridge — Manager-only (orchestrates facebook-fca-client PODs)
 */
...
app.post('/delete', auth, async (req, res) => {
    try {
        ...
        console.log(`[/delete] Full Facebook account disconnect requested for user ${userId}`);

        // 1. Delete Pods
        await deletePods(userId);

        // 2. Delete from Redis
        await redis.del(`fb_session_${userId}`);
        ...
        await MessengerSession.findOneAndUpdate({ userId: String(userId), platform: 'facebook' }, {
            $unset: { sessionData: "" },
            isActive: false
        });
...
app.post('/internal/stats', auth, async (req, res) => {
    ...
    try {
        ...
        const response = await fetch(cleanUrl, {
            ...
            body: JSON.stringify({ userId: String(userId), secret: SECRET, platform: 'facebook' })
        });
        ...
        if (!response.ok) {
            ...
            await User.findOneAndUpdate(
                ...
                {
                    $inc: { transcriptionCount: 1, fbTranscriptionCount: 1 },
...
if (isMain) {
    app.listen(PORT, async () => {
        console.log(`[manager] Facebook FCA Manager running on ${PORT}`);
        if (MODE === 'MANAGER') {

    ...
    <tr class="user-row" data-userid={user.userId}>
        ...
        <td style={{ textAlign: 'right' }}>
            <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                ...
                <button class="btn btn-sm btn-danger deactivate-btn" data-userid={user.userId} title={user.isActive ? 'Stop Pod' : 'Delete User'} style={{ width: '34px', height: '34px', padding: '0', display: 'flex', alignItems: 'center', justifyContent: 'center', background: user.isActive ? 'rgba(239, 68, 68, 0.1)' : 'rgba(107, 114, 128, 0.1)', color: user.isActive ? '#ef4444' : '#94A3B8', border: `1px solid ${user.isActive ? 'rgba(239, 68, 68, 0.2)' : 'rgba(107, 114, 128, 0.2)'}`, borderRadius: '10px' }}>
                    {user.isActive ? (

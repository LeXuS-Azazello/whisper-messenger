...
export class WhatsAppBaileysManager {
  ...
  async getQR(userId) {
    ...
    async getPairingCode(userId) {
      if (!redis) return null;
      try {
        // Trigger request to the pod first
        await redis.set(`wa_pairing_request_${userId}`, 'PENDING', 'EX', 30);
        
        // Poll for the result generated by the pod
        for (let i = 0; i < 10; i++) {
          await new Promise(resolve => setTimeout(resolve, 1000));
          const code = await redis.get(`wa_pairing_${userId}`);
          if (code) return code;
        }
        return null;
      } catch {

...
def clone_voice(req: CloneRequest, authorization: Optional[str] = Header(None)):
    ...
    print(f"[samesame-cosy] Running cross_lingual synthesis for text len={len(req.text)}")
    
    try:
        # Cross_lingual uses prompt_speech, but no prompt_text is needed!
        output = cosyvoice.inference_cross_lingual(req.text, prompt_speech_16k)
    ...
    if req.output_format == "ogg":
        ...
        os.close(fd_ogg)
        subprocess.run(["ffmpeg", "-y", "-i", temp_out, "-c:a", "libopus", "-b:a", "32k", temp_ogg], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        os.remove(temp_out)
        temp_out = temp_ogg
        mime = "audio/ogg"
    else:

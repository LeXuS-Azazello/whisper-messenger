...
- `tts_models/multilingual/multi-dataset/your_tts` — YourTTS (мультиязычный voice cloning)

Современные версии Coqui TTS bundl'ят вокодер внутри модели — отдельный `vocoder_name` больше не требуется (и не принимается конструктором).

Модели скачиваются **только** через dedicated Kubernetes Job (`samesame-downloader-job.yaml`) в PVC. Никогда не на локальной машине. Маркер-файлы предотвращают повторные скачивания при последующих деплоях.

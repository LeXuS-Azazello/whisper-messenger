...
2. Извлекает голосовой профиль из аудио отправителя.
3. Синтезирует текст ответа в его голосе из аудио отправителя.
...
CPU Оптимизация: Работает только на CPU, на процессорах без GPU. Для ускорения инференса используется принудительное выделение потоков, VAD и все возможные примочки.  (`OMP_NUM_THREADS="4"` и `torch.set_num_threads(4)`).
